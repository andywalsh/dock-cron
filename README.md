...
## Running
```sh
docker run -v /var/run/docker.sock:/var/run/docker.sock --hostname localhost -p 3167:3167 --name cronicle andywalsh/dock-cron:latest
```
...
> e.g `docker run --name cronicle --hostname cronicle-host -p 3167:3167 -e CRONICLE_base_app_url='http://cronicle-host:3012' bluet/cronicle-docker:latest`
